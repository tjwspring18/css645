...
\begin{abstract}
	...
	re-envisioning mechanization as a complex system. It also implements an
	...
	by external factors such as security environment. They imply that
	some internal factors (e.g. military culture or economic constraints)
	act as a ``brake'' on mechanization.
...
develops mechanization as a complex system best represented by
agent-based modeling. The \textbf{Model} section presents a hypothesis and
implements an agent-based model in Python to test that hypothesis.
...
aforementioned potential causes, ``choices about
mechanization are strongly associated with a state's security environment.''
Their dataset and conclusions have guided further scholarship on military
...
mechanization is a complex system. Agent-based modeling (ABM) is a more 
...
known to be randomly assigned---are the typical econometric strategy to avoid selection bias. For
...
absurd proposition. \citet{sechser2010army} lack a good control
because it probably does not exist. Reliance on selection-riddled
observational data means that econometrics are at best an incomplete
tool for studying mechanization. 
...
interactions? necessity and sufficiency distinctions? But even as-is, Figure 2 visually conveys the essential point: \textit{the system is
not tractable to decomposition}. Ascertaining cause and effect in this system
through simple observation is impossible because of endogeneity. Moreover, controlling for
confounding variables breaks down when the controlled-for variable directly
relates to the outcome of interest \citep[``post-treatment bias'',
c.f.][]{king2010hard}. One would like to perform ``causal surgery'' \citep{pearl2009causality}, i.e. modify the value at one
of the nodes and observe the resultant system state. But, as previously covered, one generally cannot conduct
...
If one takes the previous statement to be true, then a model of military
mechanization must be at least partly a cognitive model. Some agent or set of agents perceives the outside
world; processes that information; and takes action. Orthodox studies of
mechanization incorporate cognition to some degree. For example,
...
\citep{farrell1998professionalization,farrell2001transnational}. Such learning
is absent from orthodox models of military mechanization.
...
\citep{gilbert2005simulation,miller2009complex}.
